...
int scale(int hout, int wout, const kernel::base *pre, 
    ...
    const extension::base *ext, image::rgba<float> *rgba_in, image::rgba<float> *rgba_out) 
{
    int hin, win;
    rgba_in->size(&hin, &win);
	rgba_out->load(*rgba_in);
    image::rgba<float> temp;
    fprintf(stderr, "scaling from %dx%d to %dx%d\n", hin, win, hout, wout);
    if (hout*win < wout*hin) {
        if (hout < hin) {
            if (!downsample_columns(*rgba_out, hout, pre, ext, &temp)) 
                errorf(("error downsampling columns"));
        } else {
            if (!upsample_columns(rgba_out, hout, pre, ext, &temp))
                errorf(("error upsampling columns"));
        }
        if (wout < win) {
            if (!downsample_rows(temp, wout, pre, ext, rgba_out))
                errorf(("error downsampling rows"));
        } else {
            if (!upsample_rows(&temp, wout, pre, ext,rgba_out))
                errorf(("error upsampling rows"));
        }
    } else {
        if (wout < win) {
            if (!downsample_rows(*rgba_out, wout, pre, ext, &temp)) 
                errorf(("error downsampling rows"));
        } else {
            if (!upsample_rows(rgba_out, wout, pre, ext, &temp))
                errorf(("error upsampling rows"));
        }
        if (hout < hin) {
            if (!downsample_columns(temp, hout, pre, ext,rgba_out))
                errorf(("error downsampling columns"));
        } else {
            if (!upsample_columns(&temp, hout, pre, ext, rgba_out))
                errorf(("error upsampling columns"));
        }
    }
    fprintf(stderr, "  applying discrete postfilter\n");
    fprintf(stderr, "    i.e., %s*%s^-1\n", fir->name(), ifir->name());
    if (!dlti::filter(fir, ifir, rgba_out)) {
